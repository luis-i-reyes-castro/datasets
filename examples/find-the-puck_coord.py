# -*- coding: utf-8 -*-
"""
Created on Sat Aug  5 16:54:35 2017
...
output_train = np.zeros( ( num_training_samples, rows*columns) )

train_datafile='C:/Users/Jose Manuel/Documents/GitHub/datasets/examples/train_data_file_2.npy'
output_train_datafile='C:/Users/Jose Manuel/Documents/GitHub/datasets/examples/output_train_data_file_2.npy'
valid_datafile='C:/Users/Jose Manuel/Documents/GitHub/datasets/examples/valid_data_file_2.npy'
output_valid_datafile='C:/Users/Jose Manuel/Documents/GitHub/datasets/examples/output_valid_data_file_2.npy'
...
if not os.path.isfile(train_datafile):
    for row in range(rows) :
        ...
        for col in range(columns) :
            ...
            output_train[ index, index]   = 1.0

    np.save('train_data_file_2',inputs_train,allow_pickle=True)
    np.save('output_train_data_file_2',output_train,allow_pickle=True)
else:
...
if not os.path.isfile(valid_datafile):
    ...
    output_valid = np.zeros( ( num_validation_samples, rows*columns) )
    for ( index, filename) in enumerate(validation_files) :
        # Collects outputs
        row = int( filename[ 4: 6] )
        col = int( filename[-6:-4] )
        output_valid[index,row * columns + col]=1.0
        # Collects inputs
    ...
    np.save('valid_data_file_2',inputs_valid,allow_pickle=True)
    np.save('output_valid_data_file_2',output_valid,allow_pickle=True)
else:
...
layer_7= Dense(256, activation='relu')(layer_6)
layer_8= Dropout(0.5)(layer_7)

row_output = Dense( units = rows*columns, activation = 'softmax',
                    name = 'Row_Probabilities' )( layer_8 )
...
fact_gen=70 #Factor of incresing images
